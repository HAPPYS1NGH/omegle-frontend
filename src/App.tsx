...
function App() {
  ...
  const enumerateDevices = useCallback(async () => {
    try {
      ...
      const videoInputs = devices.filter(d => d.kind === 'videoinput').map(d => ({ deviceId: d.deviceId, label: d.label }));
      const audioInputs = devices.filter(d => d.kind === 'audioinput').map(d => ({ deviceId: d.deviceId, label: d.label }));
      ...
      if (!selectedVideoDeviceId && videoInputs.length > 0) {
        ...
        console.log(`[Device Enumeration] Defaulting video device to:`, videoInputs[0].deviceId);
      }
      if (!selectedAudioDeviceId && audioInputs.length > 0) {
        setSelectedAudioDeviceId(audioInputs[0].deviceId);
        console.log(`[Device Enumeration] Defaulting audio device to:`, audioInputs[0].deviceId);
      }
    } catch (err) {
  ...
  const getStream = useCallback(async (videoId: string, audioId: string) => {
    try {
      ...
      console.log(`[getStream] Requesting stream with constraints:`, constraints);
      ...
      console.log(`[getStream] Got stream. Video track:`, videoTrack, "Audio track:", audioTrack);
  ...
  useEffect(() => {
    if (selectedVideoDeviceId && selectedAudioDeviceId && !loadingDevices && !permissionError) {
    ...
    <div>
      {permissionError ? (
        <div style={{ color: 'red', margin: 16 }}>
          <p>Camera and/or microphone access denied. Please grant access to use this app.</p>
